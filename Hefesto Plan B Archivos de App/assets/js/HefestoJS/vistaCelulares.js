...
window.addEventListener("DOMContentLoaded", async (e) => {
    ...
    db.collection("Productos").where("categoria", "==", "Celular").onSnapshot(function (querySnapshot) {
        ...
        const productoSeleccionado = celularesContainer.querySelectorAll(".producto-card");
        productoSeleccionado.forEach((seleccion) => {
            seleccion.addEventListener("click", async (e) => {
                try {
                    ...
                    const producto = doc.data();
                    localStorage.setItem("pantalla-anterior","celulares");                    
