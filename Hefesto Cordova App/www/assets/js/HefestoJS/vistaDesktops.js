...
const laptopsContainer = document.getElementById("desktops-container");

const getProducto = (id) => db.collection("Productos").doc(id).get();

window.addEventListener("DOMContentLoaded", async (e) => {

    db.collection("Productos").where("categoria", "==", "PC Escritorio").onSnapshot(function (querySnapshot) {
        ...
        productoSeleccionado.forEach((seleccion) => {
            seleccion.addEventListener("click", async (e) => {
                try {
                    ...
                    const producto = doc.data();
                    localStorage.setItem("pantalla-anterior","desktops");
